...
def get_allkinds_pull_requests_fromGHWithCLI(prjectlist = 'p_list.csv', output_file='complete', output_file_W_L ='complete_WL', output_file_wo_L ='complete_WOL', output_file_backport = 'complete_backport', output_file_normal='complete_normal'):
    ...
    with open(file_path,"rU") as fp:
        ...
        for idx, line in enumerate(data_read):
            ...
            for x in range(total_pulls):
                try:
                    pull = pulls[x]

                    if pull_count>10 : break
                    ...
                    pull_info = [line[0], str(pull.number), pull.head.ref+"::"+pull.base.ref, str(intra_branch), pr_status, pr_labels, pr_discussion, all_commits]
    ...
    with open(output_file_normal, "wt") as fp5:
...
'Nixpkgs_all_PRs.csv', 
'Nixpkgs_backport_keywordsPRs.csv',
'Nixpkgs_backport_in_Discussion_OnlyPRs.csv', 
'Nixpkgs_backport_in_TitleandLabes_PRs.csv', 
'Nixpkgs_normal_PRs.csv')